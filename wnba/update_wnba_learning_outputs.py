...
def normalize_projection_frame(frame: pd.DataFrame, source_path: Path, slate_date: str) -> pd.DataFrame:
    ...
    normalized["team"] = (team_series if team_series is not None else pd.Series([""] * len(normalized), index=normalized.index)).astype(str)
    normalized["opponent"] = (opponent_series if opponent_series is not None else pd.Series([""] * len(normalized), index=normalized.index)).astype(str)
...
def load_actuals() -> pd.DataFrame:
    ...
    actuals["team"] = actuals.get("team", "").astype(str)
    actuals["opponent"] = actuals.get("opponent", "").astype(str)
    return actuals
