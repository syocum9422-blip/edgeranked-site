...
REPORT_PATH = RAW_DIR / "wnba_history_backfill_report_20260508.csv"
PLAYER_GAMES_PATH = RAW_DIR / "wnba_player_games.csv"
PLAYER_GAMES_RAW_PATH = RAW_DIR / "wnba_player_games_raw.csv"
BACKFILLED_PATH = RAW_DIR / "wnba_player_games_backfilled_20260508.csv"
...
REQUIRED_TEAMS = {"CON", "GSV", "NYL", "SEA", "TOR", "WAS"}
...
def fetch_game_logs(
    ...
    league = SOURCE_TO_LEAGUE.get(source)
    if not league:
...
def rebuild_history(existing: pd.DataFrame, fetched: pd.DataFrame, target_keys: set[str]) -> pd.DataFrame:
    kept = existing[~existing["player_name"].map(canonicalize_name).isin(target_keys)].copy()
...
def main() -> None:
    ...
    if len(rebuilt) < 2300:
        raise RuntimeError(f"Backfilled history row count is too low: {len(rebuilt)}")
    if missing_teams:
